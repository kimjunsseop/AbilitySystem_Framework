...
## 6. 주의 사항 :
- Ability 등록 필수
  TryUseAbility() 호출 시, 해당 Ability가 반드시 AbilityController의 abilities 리스트에 포함되어있어야합니다.
- TargetSelector Null 체크
  모든 Ability에는 반드시 TargetSelector를 설정해야합니다. 그렇지 않을 경우 NullException이 발생할 수 있습니다.
- Effect 실행 순서 의존성 주의
  Ability.effects 는 리스트 순서대로 진행됩니다.
- 단발성 Effect 구조
  현재 Effect는 단일 실행 구조입니다. 지속 효과 등의 처리는 별도 확장이 필요합니다.
- 쿨타임 관리
  CooldownHandler에서 Ability 쿨타임을 자동관리하므로, 별도의 쿨타임 로직을 중복구현하지 않도록 해야합니다.
- 멀티플레이 환경 미지원
  본 프레임워크는 싱글플레이 기준으로 설계되었습니다. 멀티플레이 적용 시 상태 동기화 로직이 필요합니다.
